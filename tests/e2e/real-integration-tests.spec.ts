...
test.describe('Real Integration Tests - Jump.ai Challenge', () => {
  ...
  test('LinkedIn OAuth flow works', async ({ page }) => {
    ...
    try {
      ...
      await page.screenshot({
        path: 'tests/screenshots/linkedin-oauth-success.png',
        fullPage: true,
      });
    } catch (error) {
      // If redirect fails, verify the button works
      ...
      await page.screenshot({
        path: 'tests/screenshots/linkedin-oauth-button.png',
        fullPage: true,
      });
    }
  });
  ...
  test('Google Calendar integration works', async ({ page }) => {
    ...
    const hasCalendarContent = await page
      .locator('text=Upcoming Meetings')
      .isVisible()
      .catch(() => false);
    const hasCreateButton = await page
      .locator('text=Create Meeting')
      .isVisible()
      .catch(() => false);
    ...
    await page.screenshot({
      path: 'tests/screenshots/google-calendar-integration.png',
      fullPage: true,
    });
  });
  ...
  test('LinkedIn publishing works', async ({ page }) => {
    ...
    const hasGenerateButton = await page
      .locator('text=Generate LinkedIn Post')
      .isVisible()
      .catch(() => false);
    const hasPublishButton = await page
      .locator('text=Publish to LinkedIn')
      .isVisible()
      .catch(() => false);
    ...
    if (hasGenerateButton) {
      ...
      const hasGeneratedContent = await page
        .locator('text=Generated Content:')
        .isVisible()
        .catch(() => false);
    ...
    await page.screenshot({
      path: 'tests/screenshots/linkedin-publishing-success.png',
      fullPage: true,
    });
  });
  ...
  test('Complete workflow: Meeting → Content → Publish', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'tests/screenshots/complete-workflow-success.png',
      fullPage: true,
    });
  });
});
