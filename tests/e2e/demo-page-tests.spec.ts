...
test.describe('Demo Page Tests - Jump.ai Challenge', () => {
        ...
        body: JSON.stringify({
          ...
          data: {
            posts: [
              {
                platform: 'LinkedIn',
                content: 'Just wrapped up an insightful Q4 portfolio review with a client. Key takeaways: market volatility presents opportunities for tax-loss harvesting, and diversification remains crucial for long-term success. #FinancialPlanning #PortfolioManagement',
  ...
  test('Content generation on demo page works', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/18-demo-content-generation.png', fullPage: true });
  });
  ...
  test('Publishing workflow works', async ({ page }) => {
    ...
    if (await publishButton.isVisible()) {
      ...
      console.log('✅ Publishing workflow UI verified (publish button not visible without content)');
    }
